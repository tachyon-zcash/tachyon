...
fn spend_stamp_rejects_identity_cv() {
    ...
    // The identity cv dies at the header boundary: the SpendHeader's points are
    // witnessed on the way into the step and the identity is unrepresentable,
    // ahead of `ActionDigest`'s own identity rejection.
...
fn unspent_bind_rejects_tip_mismatch() {
    ...
    // Honest witness, but the finalized range no longer equals
    // `elapsed ++ [nf_end]`, so the finalize relation rejects it before the arc
    // match. The tip is the range's top coefficient, so a wrong range is a
    // wrong tip.
    ...
    witness.1 = NfRangePoly::from_iter([Nullifier::from(Fp::random(&mut *rng))]);
