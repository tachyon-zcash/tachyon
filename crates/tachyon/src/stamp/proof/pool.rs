...
use crate::{
    ...
    relations::{
        enforce::{
            enforce_accumulator_recurrence, enforce_arc_match, enforce_poly_concat,
...
impl Step for UnspentBind {
    ...
    /// `(elapsed_poly, tip_poly, range_poly, polys, weights, accumulator,
    /// weight_quotients, accumulator_quotient, creation_epoch)`.
    type Witness<'source> = (
        NfSeqPoly,
        NfSeqPoly,
        NfSeqPoly,
    ...
    fn witness<'source>(
        ...
        let g0 = host_generators.first().ok_or(ragu::Error::InvalidWitness(
            "UnspentBind: missing host generator".into(),
        ))?;
