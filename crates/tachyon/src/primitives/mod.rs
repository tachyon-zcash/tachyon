...
pub use ragu_pasta::{EpAffine, Fp, Fq};
...
impl Anchor {
    /// Merges two anchors, returning the later (superset) state.
    ///
    /// For an append-only accumulator, the later anchor subsumes the
    /// earlier. This is "the max of all aggregated anchors."
    ///
