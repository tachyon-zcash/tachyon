...
The nullifier for epoch $e$ is the note's pronullifier coefficient shifted by the note commitment:

$$ \mathsf{nf}_e = M_e + \mathsf{cm} $$

where

- $M$ is the note's pronullifier polynomial, committed to the $\psi$ field[^commitment]
- $e$ is an epoch index
