...
//! - [`NfWindowSpectrum`] is the whitened trace $W = T + w$, bound as
...
//!
//! [`NfSboxStep`](crate::stamp::proof::delegation::NfSboxStep) links the two
//! in-circuit, $\mathsf{nf\_commit} = \mathsf{trace\_commit} +
//! \lbrack w \rbrack\,\mathcal{G}_0$: one scalar multiplication, once per
//! window.
...
use crate::{
    keys::NoteMasterKey,
    nullifier::{
        NF_DERIVATION_WIDTH, Nullifier,
        derivation::{NF_COSET_SHIFT, ROUND_SCHEDULE, WRAP_MASK_INV},
    },
...
impl NullifierTrace {
    /// The nullifier this row produces under `mk`'s whitening key.
    ///
    /// The trace stops before whitening, so its final cell is the cipher's
    /// pre-whitening output and the nullifier is that cell plus $w$.
    #[must_use]
    pub fn whiten(&self, mk: &NoteMasterKey) -> Nullifier {
        #[expect(clippy::indexing_slicing, reason = "known size")]
        let output = self.0[self.0.len() - 1];

        Nullifier::from(output + mk.1)
    }
}
...
impl NfTraceGrid {
    ...
    pub fn round_binding_spectra(
        ...
        let wrap_values: [Fp; NF_DERIVATION_WIDTH] = array::from_fn(|row| {
...
/// last-column cells are the final round states.
///
/// Wallet-only secret material.
#[derive(AsRef, Clone, Debug, From, Into)]
...
impl NfGridSpectrum {
    ...
    /// The whitened trace $W = T + w$: `mk`'s whitening key folded into the
    /// constant coefficient, so $\mathsf{nf}_{\mathsf{base}+j} = W(\sigma
    /// \zeta^j)$.
    #[must_use]
    pub fn whiten(&self, mk: &NoteMasterKey) -> NfWindowSpectrum {
        let mut whitened = self.0.clone();
        whitened += &Polynomial::from_coeffs(Vec::from([mk.1]));
        NfWindowSpectrum(whitened)
    }
}
...
/// Wallet-only secret material.
#[derive(AsRef, Clone, Debug, From, Into)]
pub struct NfWindowSpectrum(#[debug(skip)] Polynomial);

impl NfWindowSpectrum {
    /// Deterministic (untrapdoored) commitment to the polynomial.
    #[must_use]
    pub fn commit(&self) -> NfWindowSpectrumCommit {
        NfWindowSpectrumCommit(self.0.commit())
    }
}

/// Commitment to a [`NfWindowSpectrum`].
#[derive(Clone, Copy, Debug, From, Into, PartialEq, TotalEq)]
pub struct NfWindowSpectrumCommit(Eq);
