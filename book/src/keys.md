...
An $\mathbb{F}_p$ element used in nullifier derivation.[^nullifiers] With the note's trapdoor $\psi$ it seeds the note's master key $\mathsf{mk}$, from which each epoch's nullifier derives.
